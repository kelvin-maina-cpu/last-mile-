...
const PRIMARY_API_ORIGIN = import.meta.env.VITE_API_URL || null
...
export const HAS_PRIMARY_BACKEND = !!PRIMARY_API_ORIGIN

function normalizeOrigin(url) {
  return url.replace(/\/$/, '')
}

function deriveApiBaseUrl(apiOrigin) {
  if (!apiOrigin) return null

  const normalized = normalizeOrigin(apiOrigin)
  return normalized.endsWith('/api') ? normalized : `${normalized}/api`
}

function deriveSocketUrl(apiOrigin) {
  if (!apiOrigin) return null

  try {
    const url = new URL(normalizeOrigin(apiOrigin))
    if (url.pathname.endsWith('/api')) {
      url.pathname = url.pathname.replace(/\/api\/?$/, '') || '/'
    }
    return `${url.origin}${url.pathname === '/' ? '' : url.pathname.replace(/\/$/, '')}`
  } catch {
    return normalizeOrigin(apiOrigin).replace(/\/api\/?$/, '')
  }
}
...
//   - Local dev, VITE_API_URL set: '<origin>/api' (absolute)
//   - Production: '<render-backend-origin>/api'
//
// WS_URL:
//   - Local dev, no VITE_WS_URL: ws://localhost:3000 (real backend)
//   - Local dev, VITE_WS_URL set: the configured URL
//   - Production: derived from VITE_API_URL origin or VITE_WS_URL
// =============================================================================

export const API_BASE_URL = (() => {
  if (PRIMARY_API_ORIGIN) return deriveApiBaseUrl(PRIMARY_API_ORIGIN)
  if (isDeployed) return null
  return '/api' // Local dev — Vite proxy forwards to localhost:3000
})()

export const WS_URL = PRIMARY_WS_URL || deriveSocketUrl(PRIMARY_API_ORIGIN) || (isDeployed ? null : 'http://localhost:3000')
